...
public class InitialAdminUserSeeder implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent __unused__) {
        if (this.userRepository.findByUsername("wale.sebii@ey.com").isPresent())
            return;
        this.userRepository.save(new UserEntity(null, "wale.sebii@ey.com", passwordEncoder.encode("admin00"),
                "ADMIN,CONSULTANT,EXPERT,GUEST", true, true, true, true, new Date(), "wale", "sebii", "eddine",
                "0000000000", "administrator", "CIUX", "+216", true, null, null, true));
    }
}
